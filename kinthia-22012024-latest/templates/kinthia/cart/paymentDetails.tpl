				...
				<tbody>
					<tr class="linecheckout1">
						<td class="td_left_explanation_paiement">
							<!--start card payment-->
							{if isset($voyant.stripeEnable) && !empty($voyant.stripeEnable) && $voyant.stripeEnable==='1' && $voyant.paymentExpertPlatform === '1'}
							...
							{elseif isset($paymentprocessors.enabled) && !empty($paymentprocessors.enabled) && $paymentprocessors.enabled==='1' && ($voyant.paymentExpertPlatform === '0' || $voyant.paymentExpertPlatform === '')}
								<div id ="payment-option-1-container">
									<div class="c-paiement-input">
										{if isset($user.userId) && !empty($user.userId) && $user.userId > 0 || $cart.type==='TimePack'}
											<input type="radio" id="payment-option-1" class="c-radio" name="paymentMethod" value="stripe">
										{/if}
								</div>
								<label for="payment-option-1" class="c-paiement-label">
								...
								<div id="payment-option-1-additional-information" class="margin-t-15 js-additional-information" style="display: none;">
									...
									<div class="c-paiement-card">
										<form action="{'/cart/stripeSubmit'|url}" method="post" id="stripePaymentForm">
											<input type="hidden" name="type" value="{$cart.type}" />
											{if isset($voyant.stripeEnable) && !empty($voyant.stripeEnable) && $voyant.stripeEnable==='1'}
												{if isset($voyant.voyantId) && !empty($voyant.voyantId) && $voyant.voyantId > 0 || ($cart.type==='VoyantQuestion' || $cart.type==='Platform')}
												<input type="hidden" name="voyantId" value="{$voyant.voyantId}">
												{/if}
											{elseif isset($paymentprocessors.enabled) && !empty($paymentprocessors.enabled) && $paymentprocessors.enabled==='1'}
												{if isset($user.userId) && !empty($user.userId) && $user.userId > 0 || $cart.type==='TimePack'}
												<input type="hidden" name="voyantId" value="{$voyant.voyantId}">
												{/if}
											{/if}
						...
						{if isset($voyant.payPalEnableDisable) && !empty($voyant.payPalEnableDisable) && $voyant.payPalEnableDisable==='1' && $voyant.paymentExpertPlatform === '1'}
							...
							<!--end paypal payment-->
						{elseif isset($PayPalData.enabled) && !empty($PayPalData.enabled) && $PayPalData.enabled==='1' && ($voyant.paymentExpertPlatform === '0' || $voyant.paymentExpertPlatform === '')}
							<hr class="c-paiement-hr">
							<!--start paypal payment-->
							<div id ="payment-option-2-container">
								<div class="c-paiement-input">
