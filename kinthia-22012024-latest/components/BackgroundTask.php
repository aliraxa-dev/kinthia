...
abstract class BackgroundTask extends Model
{
    ...
    public function fork($path)
    {
        ...
        $query = 'POST ' . $path . ' HTTP/1.0' . $rn
                 . 'Host: ' . $_SERVER['HTTP_HOST'] . $rn
                 . 'Content-Type: application/x-www-form-urlencoded' . $rn
                 . 'Content-Length: ' . strlen($postData) . $rn
                 . 'Connection: close' . $rn . $rn . $postData;
    ...
    public function start()
    {
        ...
        for ($i = $startIndex, $itemsCount = count($this->items);
             $i < $itemsCount && !$executeNextProcess;
             $i++
        ) {
            $item = $this->items[$i];
            $this->parseItem($item);
