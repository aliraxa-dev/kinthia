...
class Template_Lite_Compiler extends Template_Lite {
	...
	public $_auto_plugins		=	array();	// stores list of plugins attached by template
...
//		$this->_func_regexp = '[a-zA-Z_]\w*';
	...
	function _compile_file($file_contents)
	{
		...
		{
			$compiled_text = '<?php require_once(\''. $this->_get_plugin_dir($key) . $key . '\'); $this->register_tmp_' . $value[0] . '("' . $value[1] . '", "' . $value[2] . '"); ?>' . $compiled_text;
		}
...
//				list (,$literal) = each($this->_literal);
				foreach($this->_literal as $literal) {
...
//				list (,$php_block) = each($this->_php_blocks);
				foreach($this->_php_blocks as $php_block) {
...
//			preg_match_all('/(?:[^\\\]' . $this->_dvar_regexp . '[^\\\])/', $variable, $_expand);
	...
	function _plugin_exists($function, $type)
	{
		...
		if (isset($this->_plugins[$type][$function]) && function_exists($this->_plugins[$type][$function]) && !isset($this->_auto_plugins[$type][$function]))
		{
