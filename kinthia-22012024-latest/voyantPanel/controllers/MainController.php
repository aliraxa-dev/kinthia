    ...
    */
    ...
    public function indexAction()
    {
        $voyant = $this->voyant->findByPk($this->userId);
            ...
            $chat_usedtime = $this->statistic->getConsultationtime($voyantId,'chat');
            $voyant['vdo_usedtime'] = $vdo_usedtime;
            $voyant['phone_usedtime'] = $phone_usedtime;
            $voyant['chat_usedtime'] = $chat_usedtime;
        ...
        $this->set('statistic', array(
            ...
            'totalEarningChat' => $this->statistic->getTotalEarning('chat',$this->userId),
    ...
    public function saveAvailableAction()
    {
        ...
        $data = array(
            ...
            'displayChat' => $display,
            'available' => $display,
            'consultationStatus' => $display == 0 ? 'OF' : 'ON',
        );
        ...
        $data = array('priority' => $this->calculatePriority($voyant));
        $this->voyant->updateByPk($data, $this->userId);
        echo $display;
        exit();
    ...
    private function calculatePriority($voyant)
    {
        $priority = rand(0, 100) % 2;
        if($voyant['consultationStatus'] == 'ON') {
    ...
    public function logOutAction()
    {
        ...
        $this->voyant->updateByPk($data, $this->userId, true);
        $c = new Criteria();
        $c->add('voyantId', $this->userId);
        $voyant = $this->voyant->find($c);
        $data = array('priority' => $this->calculatePriority($voyant));
