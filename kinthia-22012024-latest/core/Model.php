...
abstract class Model extends CoreObject
{
     ...
     * @return array
     */
    public function findAll(Criteria $criteria = null, $fields = "*", $returnModelRecords = false, $debug = false)
    {
    ...
    {  
        ...
        $prefix = Config::get("DB_PREFIX");
           return $this->db->sqlUpdate($prefix . $this->dbTable, $data, $where);
           //die(print_r($where));
    }

    public function debugUpdate($data, Criteria $c, $debug = false)
    {  
        ...
        $res = $this->db->sqlUpdate($prefix . $this->dbTable, $data, $where);
        if($debug){
            $this->db->setDebug(false);
        }
        return $res;
     ...
     */
    public function updateByPk($data, $key, $debug = false)
    {
        $c = new Criteria();
        $c->add($this->primaryKey, $key);
        $this->db->setDebug($debug);
        $res = $this->update($data, $c);
        if($debug){
            $this->db->setDebug(false);
        }
        return $res;
