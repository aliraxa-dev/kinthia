...
class Session extends CoreObject
{
    ...
    private function __construct()
    {
        ...
        $this->getConsultationChatTypeCount();
    ...
    */
    ...
    public function getConsultationChatTypeCount(){
        ...
        $userConsultation = new UserConsultationModel();
            ...
            $c->add('type', 'chat');
            $userConsultations = $userConsultation->findAll($c);
            $this->set('userChatConsultationCounts', count($userConsultations));
    }

    public function getTotalUserConsultationCounts(){

        $total = 0;
        if(isset($_SESSION['userPhoneConsultationCounts']) && !empty($_SESSION['userPhoneConsultationCounts']) || isset($_SESSION['userWebcamConsultationCounts']) && !empty($_SESSION['userWebcamConsultationCounts']) || isset($_SESSION['userChatConsultationCounts']) && !empty($_SESSION['userChatConsultationCounts'])){

            $total = $_SESSION['userPhoneConsultationCounts'] + $_SESSION['userWebcamConsultationCounts'] + $_SESSION['userChatConsultationCounts'];
