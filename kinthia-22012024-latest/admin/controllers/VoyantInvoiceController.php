...
class Admin_VoyantInvoiceController extends AppController
{
    ...
    */
    ...
    {
        $c = new Criteria();
        $c->addOrder("invoice_date DESC");
        $invoices = Model::factoryInstance('platformInvoice')->findAll($c);

        $groupedInvoices = [];
        foreach ($invoices as $invoice) {
            $invoiceDate = new DateTime($invoice['invoice_date']);
            $monthYear = $invoiceDate->format('F Y'); // Format date to "Month Year"
            $groupedInvoices[$monthYear][] = $invoice;
        }

        // show the invoices month wise...
        $this->set('invoices', $groupedInvoices);
        /*
        * Set statistic
        */
        $this->set('statistic', array(
    ...
    public function listByDateAction($monthYear)
    {
        $monthYear = urldecode($monthYear); // Decode the URL-encoded string
        $invoice_date = date('Y-m', strtotime($monthYear)); // Convert the string to a date and format it
        $invoice_start_date = date('Y-m-01 H:i:s', strtotime($monthYear)); // Get the first day of the month
        $invoice_end_date = date('Y-m-31 H:i:s', strtotime($monthYear)); // Get the last day of the month
        $c = new Criteria();
        $c->add('invoice_date', $invoice_date);
        $c->addOrder("invoice_date DESC");
        $c->addWithRelation('voyant', array(
                                'fields' => 'voyantId, name, firstName, lastName, email, address, companyTaxNumber, tva,
                                             displayNameOnInvoice, name, displayFirstNameOnInvoice,
                                             displayLastNameOnInvoice, displayTvaOnInvoice,
                                             zipCode, city'));
        $c->addWithRelation('order', array(
                                'fields' => 'orderId, voyantId, userId, purchaseDate'));
        $invoices = Model::factoryInstance('platformInvoice')->findAll($c);


        // only show those orders which are in the selected month
        $currentMonthOrders = [];
        foreach ($invoices as $invoice) {
            ...
            foreach ($orderDate as $order) {
                ...
                $orderMonthYear = $orderDate->format('Y-m-d H:i:s'); // Format date to "Month Year"
                 if ($orderMonthYear >= $invoice_start_date && $orderMonthYear <= $invoice_end_date) {
                     $currentMonthOrders[] = $order;
                 }
            }
            echo "<pre>";print_r($orderDate);
        }
        exit;


        // show the invoices month wise...
//        $this->set('invoicesList', $groupedInvoices);
        /*
        * Set statistic
        */
        $this->set('statistic', array(
