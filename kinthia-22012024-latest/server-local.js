...
connection.connect(function(err) {
  ...
  console.log("\033[41;97mConnected to database successfully \033[0m" + "\033[42;34m " + connection.config.database + " \033[0m \n");
});
...
io.on('connection',socket=>{
    ...
    connection.query('SELECT  * from  kinthiavoyance_userconsultations where `userconsultationId`= ? ',[requestId], function (error, results, fields) {
          ...
          if(results[0].status=='A' && results[0].reason=='' && results[0].start_time!=null) {
            ...
            if(voyantrecord.length>0 && userrecord.length>0) {
              console.log("\033[42;97m Connection Successfull with Socket.io \033[0m");
              ...
              console.log("\033[41;97m Connection Unsuccessfull with Socket.io \033[0m");
            }
          } else {
            console.log("\033[40;97m User not connected using Socket.io \033[0m");
          }
        });
  }
  ...
  //     console.log("\033[42;97m Register in chat \033[0m" + "\033[44;97m " + data.id + " \033[0m \n");
  //     data.socket_id = socket.id;
  //     data.socket = socket;
  //     const expert_user = userJoin(data);
  //     console.log("expert_user",expert_user);
  //     console.log("\033[42;97m Expert Chat Request ID \033[0m" + "\033[44;97m " + data.requestId + " \033[0m \n");
  //     connection.query('SELECT  * from  kinthiavoyance_userconsultations where `userconsultationId`= ? ',[data.requestId], function (error, results, fields) {
  //       if (error) throw error;
  //       if(results[0].status=='VA' && results[0].reason==null && results[0].start_time==null) {
  //         let voyantrecord = getUserById(results[0].voyantId);
  //         let userrecord = getUserById(results[0].userId);
  //         if(voyantrecord.length>0 && userrecord.length>0) {
  //           io.to(voyantrecord[0].socket_id).emit("start-jitsi");
  //           io.to(userrecord[0].socket_id).emit("start-jitsi");
  //         } else {
  //           console.log("\033[41;97m Connection Unsuccessfull with Jitsi \033[0m");
  //         }
  //       } else {
  //         console.log("\033[40;97m Connection Unsuccessfull with Jitsi \033[0m");
  //       }
  //     });
  //   });
    ...
    socket.on('register_in_chat', data=>{
        ...
        connection.query('SELECT  * from  kinthiavoyance_userconsultations where `userconsultationId`= ? ',[data.requestId], function (error, results, fields) {
            ...
            if(results[0].status=='VA' && results[0].reason==null && results[0].start_time==null) {
                ...
                let userrecord = getUserById(results[0].userId);
                if(voyantrecord.length>0 && userrecord.length>0) {
                    // Check if the user has loaded a webcam
                    if (expert_user.consultationType === 'webcam' || expert_user.consultationType === 'phone') {
                        // Start Jitsi consultation
                        io.to(voyantrecord[0].socket_id).emit("start-jitsi");
                        io.to(userrecord[0].socket_id).emit("start-jitsi");
                    } else {
                        // Start chat consultation
                        io.to(voyantrecord[0].socket_id).emit("start-chat");
                        io.to(userrecord[0].socket_id).emit("start-chat");
                    }
                } else {
                    console.log("\033[41;97m Connection Unsuccessfull with Jitsi \033[0m");
                }
  ...
  socket_stream(socket).on('file', function(stream, data) {
    console.log('register_in_chat_file.......');
  ...
  socket.on('joinRoom', (requestId) => {
    ...
    if(requestuserdetail.length>0 && requestuserdetail.length==2) {
      ...
      if (consultationtype === 'chat') {
          io.to(requestuserdetail[0].room).emit('start-chat', {msg: 'Chat Consultation', consultationType: consultationtype});
      } else {
  ...
  socket.on('start-consultation', (requestId) => {
    let requestuserdetail = getUserByRequestId(requestId);
    let voyantId = 0;
    let consultationtype = '';
    if(requestuserdetail.length>0 && requestuserdetail.length==2) {
      ...
      // Imp Code...
      connection.query('UPDATE kinthiavoyance_voyants SET consultationStatus = ?, consultationTime=? WHERE voyantId = ?', ['B',moment().tz("Europe/Paris").format('YYYY-MM-DD HH:mm:ss'),voyantId], function (error, results, fields) {
       if (error) throw error;
          connection.query('UPDATE kinthiavoyance_userconsultations SET status = ?,start_time = ? WHERE userconsultationId = ?', ['A',moment().tz("Europe/Paris").format('YYYY-MM-DD HH:mm:ss'),requestId], function (error, results, fields) {
              if (error) throw error;
              updateUserByRequestId(requestId);
              io.to(requestuserdetail[0].room).emit('start-consultation',{msg:'Start Consultation',consultationType:consultationtype});
          });
      });
    }
  });
  ...
  //   // Imp Code...
  //   connection.query('UPDATE kinthiavoyance_voyants SET consultationStatus = ?, consultationTime=? WHERE voyantId = ?', ['B',moment().format('YYYY-MM-DD HH:mm:ss'),user.voyantId], function (error, results, fields) {
  //    if (error) throw error;
  //       connection.query('UPDATE kinthiavoyance_userconsultations SET start_time = ? WHERE userconsultationId = ?', [moment().format('YYYY-MM-DD HH:mm:ss'),user.requestId], function (error, results, fields) {
  //           if (error) throw error;
  ...
  socket.on('disconnect', (reason) => {
      ...
      if((reason=='ping timeout' || reason=='transport close') && (typeof user.requestId !== "undefined")) {
        ...
        if (typeof receiver !== "undefined") {
         ...
         if(user.status=='A') {
            connection.query('UPDATE kinthiavoyance_userconsultations SET status = ?, ended_by=?, reason=?, end_time = ? WHERE userconsultationId = ?', ['E',user.type,reason,moment().tz("Europe/Paris").format('YYYY-MM-DD HH:mm:ss'),user.requestId], function (error, results, fields) {
              if (error) throw error;
         ...
         // connection.query('UPDATE kinthiavoyance_userconsultations SET status = ?, ended_by=?, reason=?, end_time = ? WHERE userconsultationId = ?', ['E',user.type,reason,moment().format('YYYY-MM-DD HH:mm:ss'),user.requestId], function (error, results, fields) {
         //  if (error) throw error;
         //    io.to(receiver.socket_id).emit('disconnect-another',{msg:reason});
         // });
         let voyantId = 0;
  ...
  socket.on('stopchat',(requestdata) => {
    ...
    connection.query('UPDATE kinthiavoyance_userconsultations SET status = ?, ended_by=?, duration=?, reason=?, end_time = ? WHERE userconsultationId = ?', ['E',requestdata.type,requestdata.talkTime,requestdata.reason,moment().tz("Europe/Paris").format('YYYY-MM-DD HH:mm:ss'),requestdata.requestid], function (error, results, fields) {
          if (error) throw error;
  ...
  socket.on('image', async image => {
    ...
    // delete images when consultation stops
    socket.on('delete-images', (requestId) => {
        console.log('delete images');
        connection.query('SELECT * FROM kinthiavoyance_voyantchats WHERE ?', [requestId], function (error, results, fields) {
            if (error) throw error;
            console.log(results);
            if (results.length > 0) {
                results.forEach(function (result) {
                    const filename = __dirname + '/uploads/chat/' + result.image;
...
httpServer.listen(PORT, ()=> console.log("\033[45;97mServer running on port \033[0m" + "\033[44;97m " + PORT + " \033[0m\n"));
