...
class Criteria extends DataAggregator
{
    ...
    private $relationsConfig = array();
    private $withRelations = array();
    ...
    {
        $this->groups[] = $this->addPrefixToValueName($group);
        return $this;
    }
    ...
    public function addWithRelation($relation, $options = array())
    {       
        $this->withRelations[$relation] = $options;   
    }
    
    public function addWithRelations($relations)
    {
        foreach($relations as $relation => $options)
        {
            $this->addWithRelation($relation, $options);
        }  
    }
    ...
    public function setRelationsConfig($relationsConfig)
    {
    ...
    public function prepareQuery()
    {
        ...
        foreach ($this->withRelations as $modelName => $withRelation) {
            $relationPrefix = "relation";
            
            if (empty($this->relationsConfig['relations'][$modelName])) {
                throw new Exception("Relation ".$this->relationsConfig['dbTable'] . "-> $modelName wasn't defined");
            }
            
            $relation = $this->relationsConfig['relations'][$modelName];
            ...
            $localKey = !empty($relation['localKey']) ? $relation['localKey'] : $foreignKey; 
            $this->addInnerJoin($modelName . 's',
                                $modelName . 's.' . $foreignKey,
                                $this->relationsConfig['dbTable'] . '.' . $localKey);
            
            foreach(preg_split("#\s*,\s*#", $withRelation['fields']) as $field) {
                $this->addColumn(self::addPrefixToValueName($modelName . 's.' . $field) . ' as ' .$relationPrefix . $modelName . $field);
            }
        }
        ...
        foreach ($this->joins as $join_table) {
            $map_keys = $this->modelMap[$join_table[0]][$join_table[1]];
            $joins[] = $join_table[0] . "." . $map_keys['local'] . "=" . $join_table[1] . "." . $map_keys['foreign'];
        }
