    ...
    <div class="card">
      ...
      <div id="orderList" class="card-body table-responsive p-0">
        <table class="table text-nowrap">
          ...
          <tbody>
            {foreach from=$cartRecordList value=list}
              <tr class="{cycle values='bg-grey-kin, '}">
                <td>{$list.creationDate|date_format:"%d/%m/%Y"}</td>
                <td>{$list.cartId}</td>
                <td>{$list.user.name}</td>
                <td>{$list.totalPriceSum}€</td>
                <td><a href="/admin/order/cartDetails/{$list.cartId}">See</a></td>
              </tr>
            {/foreach}
