...
{   
    ...
    public function detailsAction($voyantId, $urlName, $page = 1)
    {
        ...
        $chatC->add('status', 'E');
        $chatC->add('type', 'chat');
        $chatC->add('voyantId', $voyant->voyantId);
        $chatConsultations = Model::factoryInstance('userConsultation')->findAll($chatC);

        //  total consultations for phone/webcam
        $consultations = count($pConsultations) + count($cConsultations) + count($chatConsultations);
  ...
  $consultationType1 = $this->consultationType->findByPk($consultationTypeid1[0]['id']);        
    ...
    */
    ...
    public function statusAction($voyantId = 0)
    {
        ...
        if($data){
            $result = ['consultationStatus' => $data['consultationStatus'], 'displayPhone' => $data['displayPhone'], 'displayWebcam' => $data['displayWebcam'], 'displayEmail' => $data['displayEmail'], 'displayChat' => $data['displayChat']];
        }
        else{
            $result = ['consultationStatus' => '', 'displayPhone' => '', 'displayWebcam' => '', 'displayEmail' => '', 'displayChat' => ''];
        }
