...
class CartController extends AppController
{
    ...
    public function init()
    {
        ...
        $this->acl->allow('guest', $this->name, 'confirmation');
        ...
        if (!$this->acl->isAllowed($this->session->get('role'), $this->name, $this->action) && $this->action!='pack' && $this->action!='addPack' && $this->action !== 'confirmation'){
    ...
    */
    public function paymentDetailsAction($voyantId, $packageId, $type)
    {
        ...
        $this->set('PayPalData', $this->PaymentProcessor->findByPk("PayPal"));
    ...
    public function processPaymentOption($voyant, $order, $type)
    {
        if(isset($order->status)){
            $this->set('latestOrderStatus',$order->status);
        }
        if ('payPal' === $this->request->paymentMethod) {
            ...
            if(($type==='VoyantQuestion' || $type==='Platform') && $voyant->paymentExpertPlatform==1){
                    if ($voyant->payPalEnableDisable == 1) {
                ...
                $payment = Model::factoryInstance('paymentprocessor')->findByPk('PayPal');
                if ($payment['enabled'] == 1) {
                    $semail = $payment->email;
                } else {
                    $semail = '';
                }
    ...
    public function stripeSubmitAction()
    {
        ...
        try {
            ...
            if ($submit) {
                $c = new Criteria();
                $c->add('userQuestions.orderId', $order->orderId);
                $c->addWithRelation('voyant', array('fields' => 'voyantId, paidBackQuestion, tva, displayTvaOnInvoice'));
                $c->addWithRelation('order', array('fields' => 'type'));
                $c->addWithRelation('voyantQuestion', array('fields' => 'price'));

                $orderDetails = $this->userQuestion->findAll($c);
                $voyantQPercentage = 0;
                $vatExclude = 0;
                foreach($orderDetails as $order) {
                    $vatExclude = 1 + ($order['voyant']['tva'] / 100);
                    $voyantQPercentage = $order['voyant']['paidBackQuestion'];
                    $questionPrice = $order['price'];
                    $priveVatExcluded = ($questionPrice / $vatExclude);
                    if ($order['type'] === 'Voyant' && $order['voyant']['displayTvaOnInvoice'] === '1') {
                    ...
                    // $expertEarning = ($voyantQPercentage / 100) * $priveVatExcluded;
                    $whoispaid = $order['type'];
                    $updateData = array('expertQEarning' => $expertEarning);
                    $paidData = array( 'whoIsPaid' => $whoispaid);
                    $userQuestionCriteria = new Criteria();
                    $userQuestionCriteria->add('userQuestions.orderId', $order['orderId']);
                    $userQuestionCriteria->add('userQuestions.voyantQuestionId', $order['voyantQuestionId']);
                    $userQuestionCriteria->add('userQuestions.userId', $order['userId']);
                    $this->userQuestion->update($updateData, $userQuestionCriteria);
                    $this->userQuestion->update($paidData, $userQuestionCriteria);
    ...
    public function confirmationAction()
    {
        ...
        $this->set('redirectUrl', Config::get('siteRootUrl') . AppRouter::getRewrittedUrl('/cart/confirmation', false));
        $this->set('message', 'Your order has been successfully placed.');

        $this->questionPaymentProcess();
    }

    public function questionPaymentProcess()
    {
        ...
        $question = Model::factoryInstance('userQuestion')->find($c);

        $voyant = Model::factoryInstance('voyant')->findByPk($question->voyantId);

        $a = new Criteria();
        $a->add('itemId', $question->voyantQuestionId);

        $cartItems = Model::factoryInstance('cartItem')->findAll($a);


        $vatExclude = 1 + ($voyant->tva / 100);
        $voyantQPercentage = $voyant->paidBackQuestion;
        $questionPrice = $cartItems[0]['totalPrice'];
        $priveVatExcluded = ($questionPrice / $vatExclude);
        if ($question->whoIsPaid === 'Voyant' && $voyant->displayTvaOnInvoice === '1') {
            // Expert receives 100% of the money, including VAT
            $expertEarning = $questionPrice;
        } else {
            // Calculate the expert's earning based on the percentage
        ...
        // update user question table
        $updateData = array('expertQEarning' => $expertEarning);
        $userQuestionCriteria = new Criteria();
        $userQuestionCriteria->add('userQuestions.orderId', $question->orderId);
        $userQuestionCriteria->add('userQuestions.voyantQuestionId', $question->voyantQuestionId);
        $userQuestionCriteria->add('userQuestions.userId', $question->userId);

        $this->userQuestion->update($updateData, $userQuestionCriteria);

    }

    public function createUserQuestion($order) {
        if (!empty($order)) {
            $type = $order->type;
            $type = $type === 'Voyant' ? 'VoyantQuestion' : 'Platform';




            $a = new Criteria();
            $a->add('cartId', $order->cartId);
            $a->add('type', 'VoyantQuestion');
            $a->add('voyantId', $order->voyantId);
            $cartItems = Model::factoryInstance('cartItem')->findAll($a);
            $voyantQuestionId = $cartItems[0]['itemId'];

            $voyant = Model::factoryInstance('voyant')->findByPk($order->voyantId);

            $userQuestion = new UserQuestionRecord();
            $userQuestion->orderId = $order->orderId;
            $userQuestion->userId = $order->userId;
            $userQuestion->voyantId = $voyant->voyantId;
            $userQuestion->voyantQuestionId = $voyantQuestionId;
            $userQuestion->whoIsPaid = $type === 'VoyantQuestion' ? 'Voyant' : 'Platform';
