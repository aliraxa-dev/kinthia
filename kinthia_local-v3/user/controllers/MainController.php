    ...
    */
    ...
    public function logInAction($urlName = NULL)
    {
        ...
        if (isset($this->request->email) && isset($this->request->password)) {
            ...
            $rememberMe = isset($this->request->rememberMe) ? $this->request->rememberMe : true;
            $response = $this->loginAsUser($email, $this->request->password, $rememberMe, $this->request->urlName, true);
    ...
    private function loginAsUser($email, $password, $rememberMe = TRUE, $urlName, $redirect = false)
    {
        if (!$this->bannedEmail->isBanned($email)
                ...
                if ($redirect) {
                    if (!empty($urlName)) {
                        if ($this->isAjaxRequest()) {
                            $this->jsonResponse(['success' => true, 'url' => '/user/management', 'action' => 'pass']);
                        } else {
                            $this->redirect(AppRouter::getRewrittedUrl("/" . $urlName));
                        }
                        exit;
                    }
    ...
    // Method to check if the request is AJAX
