                ...
                {foreach from=$order.orderItems item=orderItem}
                    {if $orderItem.itemId}
                        {if $itemVal != $orderItem.itemId}
                            ...
                            {else}
                                <strong>Title not found</strong><br><br>
                            {/if}
                        ...
                        {foreach from=$order.orderItems item=orderItem}
                            ...
                            {if $itemVal != $orderItem.itemId}
                               {$orderItem.name}
                                <br/>
                                <br/>
                                {assign var=itemVal value=$orderItem.name}
                            {/if}
                            {/if}
                        {/foreach}
                ...
                <!-- Private Messages -->
                    <td class="td_left">
                        ...
                        {foreach from=$order.orderItems item=orderItem}
                            {if $orderItem.itemId}
                            {if $itemVal != $orderItem.itemId}
                                <a href="{'/user/privateMessage/details/'.$orderItem.voyantId}" class="link_panel_grey_underline" title="{'orderIndex_contact_voyant_title'|lang} {$order.voyant.name}">{'Sent / See'|lang}</a>
                                <br/>
                                <br/>
                                {assign var=itemVal value=$orderItem.itemId}
                            {/if}
